...
const ProductSquare = ({ image, url, name, width, height, weight, colors, price, description, intl: { formatNumber } }: Props) => {
  ...
  return (
    <Tag to={url} className={`${styles.container} ${url && styles.containerWithHover}`}>
      ...
      {price && <div className={styles.price}>{formatNumber(price, { style: 'currency', currency: 'EUR' })} + IVA</div>}
    </Tag>
  )
}
